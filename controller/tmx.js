...
const updateTmxPodStatus = ((req,res)=>{
    ...
    tmxPodDB.findOneAndUpdate(filter, update, option, (err, docs) => {
        if (err) {
            console.log("Error on updating the information on database")
            console.log(err)
            res.render('error', {
...
const financeAcknowledgeTmx = ((req,res)=>{
    let data = req.body
    let pod_id = data.pod_id
    let acknowledge = "T"
    let filter = {pod_id}
