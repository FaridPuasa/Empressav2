...
const insertPodPersonal = ((req,res)=>{
    ...
    pod.save((err,doc)=>{
        if (err) {
            console.log("Error on Creating FMX POD")
            console.log(err)
            res.render('error', {
                title: "Error",
                code: '404',
...
const updatePersonalPodStatus = ((req,res)=>{
    ...
    personalPodDB.findOneAndUpdate(filter, update, option, (err, docs) => {
        if (err) {
            console.log("Error on updating the information on database")
            console.log(err)
            res.render('error', {
...
const financeAcknowledgePersonal = ((req,res)=>{
    let data = req.body
    let pod_id = data.pod_id
    let acknowledge = "T"
    let filter = {pod_id}
...
const updatePersonalSelf = ((req,res) =>{
    let data = req.body
    let date =  moment().format("DD/MM/YYYY")
